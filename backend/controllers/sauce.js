...
exports.likedNoliked = (req, res, next) => {
  if (req.body.like === 1) {
    Sauce.updateOne({ _id: req.params.id }, { $set: { likes: 1 }, $push: { usersLiked: req.body.userId } })
      .then(() => res.status(200).json({ message: 'Like ajouté !' }))
      .catch(error => res.status(400).json({ error }))
  } else if (req.body.like === -1) {
    Sauce.updateOne({ _id: req.params.id }, { $set: { dislikes: -1 }, $push: { usersDisliked: req.body.userId } })
